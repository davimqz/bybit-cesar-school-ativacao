    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-slate-600">
          Deposite CSR e receba mais CSR com o tempo. Antes de olhar o percentual,
          olhe a reserva: todo rendimento sai de algum lugar, e aqui você consegue
          ver exatamente de onde e por quanto tempo ainda.
        </p>
      </div>
...
function DeOndeVem() {
  const { reserva, segundosDeReserva, taxaPorSegundo, totalEmStake, aprBps } = useStaking();
  ...
  return (
    ...
    >
      <dl className="grid gap-5 sm:grid-cols-4">
        ...
        />
        <Stat rotulo="Total depositado" valor={fmt(totalEmStake, 0)} sufixo="CSR" />
      </dl>

      <div className="mt-6 grid gap-5 border-t border-slate-200 pt-6 sm:grid-cols-2">
        <Stat
          rotulo="APR (o que o contrato faz)"
          valor={fmtPct(aprBps !== undefined ? Number(aprBps) / 10_000 : undefined)}
          tom="alerta"
      ...
      {secando && (
        <div className="mt-4">
          <Aviso tom="erro">
            A reserva está no fim. Quando ela zerar, o rendimento vira zero — e o APR
            também, porque o contrato para de emitir. Avise o professor.
          </Aviso>
        </div>
      )}

      <NotaDeAula>
        Esse APR é absurdo de propósito, e é a lição mais útil deste lab: número alto
        não é generosidade, é <strong>emissão dividida por pouca gente</strong>. Quando
        alguém grande depositar, ele cai na sua frente sem ninguém te avisar. E quando a
        reserva secar, ele vira zero. Sempre pergunte quem está pagando a conta — aqui
        foi o professor, e o depósito dele está{" "}
        {link ? (
        ...
        . Em protocolos que prometem 10.000%, a resposta costuma ser: um token recém
        emitido que ninguém quer comprar.
      </NotaDeAula>
    </Card>
  );
}
...
function MinhaPosicao() {
  const { minhaPosicao, pendente, minhaFatiaBps, taxaPorSegundo } = useStaking();
  ...
  const dentro = (minhaPosicao ?? 0n) > 0n;

  return (
    <Card titulo="Sua posição" subtitulo="O pendente sobe sozinho — não precisa recarregar">
      {!dentro ? (
        <Aviso tom="info">
          Você não tem nada depositado. Deposite ao lado e este painel começa a contar.
        </Aviso>
      ) : (
        <dl className="grid gap-5 sm:grid-cols-2">
          ...
          <Stat
            ...
            valor={fmtPct(minhaFatiaBps !== undefined ? Number(minhaFatiaBps) / 10_000 : undefined)}
            dica="é ela que define quanto da emissão é sua"
          />
          <Stat rotulo="Rendendo" valor={fmt(porMinuto, 4)} sufixo="CSR/min" />
        </dl>
      )}

      <NotaDeAula>
        Repare que sua fatia — e não o seu depósito — é o que determina o rendimento.
        Depositar o dobro não dobra o ganho se o cofre triplicar de tamanho no mesmo
        dia. Rendimento em DeFi é quase sempre uma disputa por uma emissão fixa.
      </NotaDeAula>
    </Card>
  );
}

function Operacoes({ conectado, onFeito }: { conectado: boolean; onFeito: () => void }) {
  ...
  const precisaAprovar = depositando && valor !== undefined && (allowanceCsr ?? 0n) < valor;
  const semSaldo = depositando && valor !== undefined && (saldoCsr ?? 0n) < valor;
  const semPosicao = !depositando && valor !== undefined && (minhaPosicao ?? 0n) < valor;
  ...
  return (
    <Card titulo="Operar" subtitulo="Depositar, retirar, colher e reinvestir">
      ...
      <div className="space-y-4">
        ...
        {semSaldo && (
          <Aviso tom="alerta">Saldo de CSR insuficiente. Pegue mais no faucet da aula.</Aviso>
        )}
        {semPosicao && (
          <Aviso tom="alerta">Você não tem esse tanto depositado para retirar.</Aviso>
        )}
        ...
        <div className="flex flex-wrap gap-3">
          {precisaAprovar && (
            ...
            >
              {txAprovar.ocupada ? "Processando…" : "Aprovar CSR"}
            </Botao>
          )}
          <Botao
            disabled={
              !conectado || !valor || precisaAprovar || semSaldo || semPosicao || txOperar.ocupada
            }
            onClick={async () => {
              if (!valor) return;
          ...
          >
            {txOperar.ocupada ? "Enviando…" : depositando ? "Depositar" : "Retirar"}
          </Botao>
        </div>
        ...
        <StatusTx
          tx={txOperar}
          sucesso={depositando ? "Depositado. Seu rendimento já começou." : "Retirado."}
        />
        ...
        <div className="space-y-3 border-t border-slate-200 pt-4">
          <div className="flex flex-wrap gap-3">
            ...
            >
              {txColher.ocupada ? "Colhendo…" : `Colher ${fmt(pendente, 4)} CSR`}
            </Botao>
            <Botao
          ...
          <StatusTx tx={txReinvestir} sucesso="Rendimento virou depósito. Sua fatia cresceu." />

          <NotaDeAula>
            <strong>Reinvestir</strong> não move token nenhum: a recompensa já estava neste
            contrato, e a operação só aumenta a sua fatia. É exatamente essa diferença que
            separa APR de APY — e o ganho extra de quem reinveste sai do bolso de quem
            esquece de fazer isso, porque a emissão total não muda.
          </NotaDeAula>
        </div>
      </div>
    </Card>
