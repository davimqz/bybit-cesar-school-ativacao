...
import { Card, Stat, Botao, CampoValor, NotaDeAula, Aviso } from "@/components/ui";
...
export function SwapCard({
  ...
  const [recebe, semImpacto, slippageBps] = (previsao as readonly [bigint, bigint, bigint] | undefined) ?? [];
  ...
  useEffect(() => {
    ...
    onPrevisto?.(vendendoCsr ? { x: x + dx, y: y - dy } : { x: x - dy, y: y + dx });
  }, [reservas, valor, recebe, vendendoCsr, onPrevisto]);

  const precisaAprovar = valor !== undefined && (allowanceEntrada ?? 0n) < valor;
  const saldoInsuficiente = valor !== undefined && (saldoEntrada ?? 0n) < valor;

  const minimoAceito =
    recebe !== undefined ? (recebe * BigInt(10_000 - toleranciaBps)) / 10_000n : undefined;
  ...
  return (
    <Card titulo="Trocar" subtitulo="O preço não é seu: é a razão entre as reservas">
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <Botao variante="secundario" onClick={() => setVendendoCsr((v) => !v)}>
        ...
        <CampoValor
          ...
          onMax={() => saldoEntrada && setQuantia(formatUnits(saldoEntrada, 18))}
        />

        {valor && !erroPrevisao && recebe !== undefined && (
          <>
            <dl className="grid grid-cols-2 gap-4 rounded-xl bg-slate-50 p-4">
              <Stat rotulo="Você recebe" valor={fmt(recebe)} sufixo={tokenSaida.symbol} tom="bom" />
              <Stat
                rotulo="Slippage"
              ...
              <Stat
                ...
                valor={
                  precoEfetivo
                    ? precoEfetivo.toLocaleString("pt-BR", { maximumFractionDigits: 4 })
                    : "—"
                }
                sufixo={`${tokenSaida.symbol}/${tokenEntrada.symbol}`}
              />
              <Stat rotulo="Mínimo aceito" valor={fmt(minimoAceito)} sufixo={tokenSaida.symbol} />
            </dl>
        ...
        {erroPrevisao && valor && (
          <Aviso tom="erro">
            O pool não consegue executar esse tamanho. Geralmente significa que você está
            tentando levar mais do que existe na reserva.
          </Aviso>
        )}

        {saldoInsuficiente && <Aviso tom="alerta">Saldo insuficiente de {tokenEntrada.symbol}.</Aviso>}

        <div className="flex flex-wrap gap-3">
          {precisaAprovar && (
            <Botao
              disabled={!isConnected || txAprovar.ocupada || saldoInsuficiente}
              onClick={async () => {
            ...
            >
              {txAprovar.ocupada ? "Aprovando…" : `1. Aprovar ${tokenEntrada.symbol}`}
            </Botao>
          )}

          <Botao
            disabled={
              !isConnected || !valor || precisaAprovar || saldoInsuficiente || txSwap.ocupada
            }
            onClick={async () => {
          ...
          >
            {txSwap.ocupada ? "Trocando…" : precisaAprovar ? "2. Trocar" : "Trocar"}
          </Botao>
        </div>

        <StatusTx tx={txAprovar} sucesso="Aprovado. Agora o swap." />
        <StatusTx tx={txSwap} sucesso="Troca executada." />

        <NotaDeAula>
          <strong>Aprovar</strong> e <strong>trocar</strong> são duas transações porque um
          contrato não pode simplesmente pegar seus tokens: você precisa autorizar antes. É o
          mesmo <code>approve</code> de qualquer DEX real — e a razão pela qual aprovações
          antigas e esquecidas são um risco de segurança.
        </NotaDeAula>
      </div>
    </Card>
  );
